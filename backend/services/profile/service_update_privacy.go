...
func (s *ProfileService) UpdatePrivacy(userID string, requestorID any, wantedStatus string) *models.ErrorJson {
	...
	if userID != requestorID {
		return &models.ErrorJson{Status: 403, Error: "You can't update another user's profile"}
	}
	...
	if visibility == wantedStatus {
		return &models.ErrorJson{Status: 403, Error: fmt.Sprintf("Your account is already %s", wantedStatus)}
	}
